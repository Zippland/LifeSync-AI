...
def email_advice_with_ai(advice_part, data, ai_version, present_location, user_career, local_time, schedule_prompt=""):
    ...
    try:
        ...
        parts = {
            ...
            "4": f"请在一个h2段落内总结：如果未来有需要提前花时间准备的任务，而且预计今天已经在准备周期中，请提醒我。反之，如果今天不在准备周期，就不用说。",
        ...
        if advice_part == "1":
            ...
            system_content = f"GPT应当表现出作为私人秘书的能力，根据雇主的安排提醒他注意事项。现在时间是{local_time}。请在汇报时把所有内容浓缩为一句话，不要让雇主今天做，今天已没有时间，只用提醒他注意，并使用中文。请用HTML格式（不要CSS），只要body部分，包括一个h2主标题和其余内容。不要任何寒暄，不要任何称呼，不要任何问候语或开场白。"
            prompt += f"\n{data}。\n\n"
