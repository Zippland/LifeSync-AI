...
def email_advice_with_ai(data, ai_version, present_location, user_career, local_time, schedule_prompt=""):
    ...
    try:
        ...
        2. 时间安排建议：根据现实作息和工作时间，请严格遵守雇主需求：如果没有必须安排，那么{schedule_prompt}，提出任务时间安排建议，记得留时间出来吃饭休息。现在时间是{local_time}，请提供具体、美观且详细的时间轴，同时把任务详情和注意事项用小字写在时间轴里面，并详细告诉我具体怎么做。
        ...
        system_content = f"GPT应当表现出作为私人秘书的能力，向雇主做当天内接下来时间的汇报，提醒他根据天气情况做相应的准备。现在时间是{local_time}。请严格遵守雇主需求：{schedule_prompt}，对于end date不是今天的任务，如果时间安排不满足雇主作息要求，就不要安排在今天。请在汇报时体现出秘书的专业性和对他的家人般的关心，并使用中文。请用HTML格式（不要CSS），只要body部分，包括一个h2主标题和其余内容。不要任何寒暄，不要任何称呼，不要任何问候语或开场白。"
