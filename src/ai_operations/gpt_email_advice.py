...
def email_advice_with_ai(advice_part, data, ai_version, present_location, user_career, local_time, schedule_prompt=""):
    ...
    try:
        ...
        if advice_part == "1":
            ...
            system_content = f"GPT应当表现出作为私人秘书的能力，根据现在的时间和未完成的工作，向雇主做当天内接下来时间的汇报，协助他规划一整天的时间安排。雇主是的职业是{user_career}，住在{present_location}，现在时间是{local_time}。请在汇报时体现出秘书的专业性和对任务细节的理解能力，并使用中文。请用HTML格式（不要CSS），只要body部分，包括一个h2主标题和具体时间轴。不要任何寒暄，不要任何称呼，不要任何问候语或开场白。"
