...
def email_advice_with_ai(data, ai_version, present_location, user_career, local_time, schedule_prompt=""):
    ...
    try:
        ...
        1. 天气情况和建议：简要概括明天的天气情况及变化，然后并生成以及明天天的穿搭建议、出行建议及注意事项（可能包括穿搭、防晒、雨具、防风等等）。
        2. 明日任务提醒和建议（越详细越好，不用规定时间）：总结当天未完成的任务，并提醒明天要做的任务（不用规定时间）。同时用小字备注任务详情和注意事项，为用户写提示以更好地理解任务。
